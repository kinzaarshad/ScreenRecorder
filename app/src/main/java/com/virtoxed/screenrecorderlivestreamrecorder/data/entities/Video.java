package com.virtoxed.screenrecorderlivestreamrecorder.data.entities;
...
public class Video implements Parcelable {
    ...
    public Video(String title, long duration, int bitrate, int fps, int width, int height, long size, String localPath, String cloudPath, String thumbnailLink) {
        mTitle = title;
        mDuration = duration;
        mBitrate = bitrate;
        mFps = fps;
    ...
    public static ArrayList<Video> createTempVideoFromGoogleDriveData(List<GoogleDriveFileHolder> files) {
        ...
        for(GoogleDriveFileHolder file: files){
            videos.add(new Video(file.getName(), 0, 0, 0, 0,0, file.getSize(), "", file.getId(), file.getThumbnailLink()));
        }
        return videos;
    }
